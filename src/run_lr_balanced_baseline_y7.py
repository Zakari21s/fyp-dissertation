"""
Train and evaluate Logistic Regression baseline with class_weight="balanced" for disk failure prediction using y_7.

This script trains a balanced Logistic Regression model on data_splits datasets and evaluates it on validation and test sets.
It uses the same preprocessing, feature selection, and row-capping policy as the unbalanced LR baseline.
"""
...
class ModelResults:
    ...
    best_threshold: Optional[float] = None  # For LR, best threshold found on VAL
...
class BaselineResults:
    ...
    experiment_name: str = ""
    label_column: str = "y_7"
    class_weight: str = "balanced"  # Explicitly document balanced weights
    train_rows: int = 0
...
def setup_logging(log_dir: Path, log_level: str, experiment_name: str) -> logging.Logger:
    ...
    log_file = log_dir / f"lr_balanced_y7_{experiment_name}.log"
...
def load_split_data_iterative(
    ...
    Args:
        ...
        label_col: Label column name (e.g., 'y_7')
...
def train_baselines(
    ...
    """
    Train and evaluate baseline models with balanced class weights.
    ...
    results = BaselineResults(experiment_name=experiment_name, label_column=label_col, class_weight="balanced")
    
    logger.info("=" * 60)
    logger.info("TRAINING BALANCED LOGISTIC REGRESSION BASELINE")
    logger.info("=" * 60)
    logger.info(f"Experiment: {experiment_name}")
    logger.info(f"Label column: {label_col}")
    logger.info(f"Class weight: balanced")
    ...
    train_df = load_split_data_iterative(train_dir, max_train_rows, seed, label_col, logger)
    
    if len(train_df) == 0:
        raise ValueError("No training data loaded")
    
    train_X, train_y = build_xy(train_df, label_col, logger)
    ...
    lr_model = LogisticRegression(
        class_weight='balanced',  # Explicitly set balanced weights
        solver='liblinear',
        max_iter=200,
        random_state=seed
    )
    ...
    val_df = load_split_data_iterative(val_dir, max_val_rows, seed, label_col, logger)
    
    if len(val_df) == 0:
        logger.warning("No validation data loaded")
    else:
        val_X, val_y = build_xy(val_df, label_col, logger)
        ...
        # Evaluate Logistic Regression on validation (threshold 0.5)
    ...
    test_df = load_split_data_iterative(test_dir, max_test_rows, seed, label_col, logger)
    
    if len(test_df) == 0:
        logger.warning("No test data loaded")
    else:
        test_X, test_y = build_xy(test_df, label_col, logger)
        ...
        # Evaluate always-negative on test
        ...
        # Evaluate Logistic Regression on test
...
def write_reports(results: BaselineResults, logger: logging.Logger) -> None:
    ...
    # JSON report
    json_path = reports_dir / f"baseline_y7_lr_balanced_results_{experiment_name}.json"
    ...
    # Markdown report
    md_path = reports_dir / f"baseline_y7_lr_balanced_results_{experiment_name}.md"
    with open(md_path, 'w') as f:
        f.write(f"# Baseline Model Results (Balanced LR) - {experiment_name}\n\n")
        ...
        # Logistic Regression
        f.write("### Logistic Regression (Balanced)\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train and evaluate balanced Logistic Regression baseline for disk failure prediction'
    )
    parser.add_argument(
        '--experiment',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default='y_7',
        help='Label column name (default: y_7)'
    )
    parser.add_argument(
        '--max_train_rows',
        type=int,
    ...
    # Input directory (data_splits instead of data_ml)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("SUCCESS: Balanced baseline training complete")
        logger.info("=" * 60)
