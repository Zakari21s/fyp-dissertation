"""
Train and evaluate Random Forest baseline for disk failure prediction.
...
def setup_logging(log_dir: Path, log_level: str, experiment_name: str, label_col: str, balanced: bool) -> logging.Logger:
    """Setup logging configuration."""
    log_dir.mkdir(parents=True, exist_ok=True)
    balanced_suffix = "_balanced" if balanced else ""
    log_file = log_dir / f"rf_baseline_{label_col}{balanced_suffix}_{experiment_name}.log"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train and evaluate Random Forest baseline for disk failure prediction'
    )
    parser.add_argument(
        '--label',
        type=str,
        required=True,
        help='Label column name (e.g., y_30, y_14, y_7)'
    )
    parser.add_argument(
        '--experiment',
        type=str,
    ...
    logger = setup_logging(log_dir, config['logging']['level'], args.experiment, args.label, args.balanced)
